      ...
      public List<Warehouse> ViewWarehouseLocation()
        ...
        public bool AddWarehouseStockDetails(Items item)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = $"INSERT INTO WarehouseStockDetails([ItemId],[Quantity],[BatchNumber],[CreatedDate])  VALUES('{item.itemId}','{item.quantity}','{item.batchSeries}','{DateTime.Now.ToShortDateString()}')";
